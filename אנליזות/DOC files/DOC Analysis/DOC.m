...
% Calculates the DOC of the input data X
...
normalized_results = X./sum(X,1); % Normalization to relative abundances

m = size(X,2); % Number of samples
overlap = zeros(m);
dissimilarity = zeros(m);

k = 0;
overlap_vector = zeros(m*(m-1)/2,1);
dissimilarity_vector = zeros(m*(m-1)/2,1);
    ...
    case 'rjsd'
        ...
        for i = 1:m
            for j = 1+i:m
                ...
                overlap(i,j) = O(normalized_results(Shared_species1,i),normalized_results(Shared_species1,j));
